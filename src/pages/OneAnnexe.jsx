...
function OneAnnexe() {
  ...
  const fetchOne = async (id) => {
      const url = `${process.env.REACT_APP_API_URL}annexes/${id}`;
      try {
    ...
    const handleReturn = async () => {
      navigate("/bibliotheques");
    };

return (
  <div className="divOneArticle oneAnnexe" style={article.media && !article.text ? {maxWidth: "1200px"} : {}}>
      <h1>{article.titre}</h1>
      ...
      <div className={article.media ? (article.text ? 'articleImage' : 'articleJustImage') : 'article' }>
            ...
            {article.text ?
              ...
              <div>
                {article.quote && <p className='quote'>{article.quote}</p>}
                {article.source && <p className='source'>Source: {article.source}</p>}
              </div>
            }
        </div>
  </div>
